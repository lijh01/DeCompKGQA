...
class QA_baseline(nn.Module):
    ...
    def getQuestionEmbedding(self, question_tokenized, attention_mask):
        lm_last_hidden_states = self.lm_model(question_tokenized.cuda(), attention_mask=attention_mask.cuda())[0]
...
class QA_MultiQA(QA_baseline):
    ...
    def score_entity(self, head_embedding, tail_embedding, relation_embedding, time_embedding):
        lhs = head_embedding[:, :self.tkbc_model.rank], head_embedding[:, self.tkbc_model.rank:] #2,1,512
        rel = relation_embedding# 1,521
        time = time_embedding# 1,512
    ...
    def score_given_tail(self, head_embedding, tail_embedding, relation_embedding, time_embedding):
        lhs = head_embedding[:, :self.tkbc_model.rank], head_embedding[:, self.tkbc_model.rank:]
        rel = relation_embedding
        time = time_embedding
        ...
        #right = tail_embedding[:, :self.tkbc_model.rank], tail_embedding[:, self.tkbc_model.rank:]
        right = self.tkbc_model.embeddings[0].weight
        right = right[:, :self.tkbc_model.rank], right[:, self.tkbc_model.rank:]
        rt = rel[0] * time[0], rel[1] * time[0], rel[0] * time[1], rel[1] * time[1]
        full_rel = rt[0] - rt[3], rt[1] + rt[2]
    ...
    def score_given_time(self, head_embedding, tail_embedding, relation_embedding, time_embedding):
        lhs = head_embedding[:, :self.tkbc_model.rank], head_embedding[:, self.tkbc_model.rank:]
        rel = relation_embedding
        time = time_embedding
    ...
    def score_given_head(self, head_embedding, tail_embedding, relation_embedding, time_embedding):
        lhs = tail_embedding[:, :self.tkbc_model.rank], tail_embedding[:, self.tkbc_model.rank:]
        rel = relation_embedding
        time = time_embedding
        ...
        #right = head_embedding[:, :self.tkbc_model.rank], head_embedding[:, self.tkbc_model.rank:]
        right = self.tkbc_model.embeddings[0].weight
        right = right[:, :self.tkbc_model.rank], right[:, self.tkbc_model.rank:]
