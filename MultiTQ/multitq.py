...
def enrich_entities_with_time(inference, tkgqa_candidates, query_str):
    entities = []
    try:
        f, p = inference.extract_content(query_str)  # e.g., f = 'get_tail_entity', p = [head_phrase, rel, time]
    except Exception as e:
        print(f"[ERROR] Failed to extract function and params from query_str: {e}")
        return entities

    for entity_name, _ in tkgqa_candidates:
        if f == 'get_tail_entity':
            ...
            print(f"[WARN] Unsupported function type: {f}")
            continue

        quadruples = inference.find_time_for_entity(head, tail)  # shape: (num, 4)
        if quadruples is not None and len(quadruples) > 0:
            for quad in quadruples:
                entities.append(tuple(quad)) 
        #else:
            #print(f"[WARN] No quadruples found for ({head}, {tail})")
...
class Inference():
    ...
    def find_time_for_entity(self, head, tail):
        return self.get_data_compare(h=head, t=tail, n=True)
        ...
        if len(self.history_candidate_actions)>0:
            for a in self.history_candidate_actions[-1]:
                if a[1:-1] in response:
                    response = a
        f, params = self.extract_content(response)
        params = [p[1:] if p.startswith(" ") else p for p in params]
        p = params
        entities = []
        kg_res = self.query_knowledge_graph(f, p)
        if len(kg_res) == 0:
            if(f == 'get_time' or f == 'get_head_entity' or f == 'get_tail_entity'):
                self.history.append([f"{response} is a wrong action, no results in kg_res, Never ask for {response} again."])
            elif(f == 'get_before' or f == 'get_after' or f == 'get_between' or f == 'get_first' or f == 'get_last'):
                self.history.append(["No results in kg_res, try to reget the entities that you need here"])
            else:
                return
